...
class ElytronHttpsConfigurator(private val profile:String = "" ) : WildflyHttpsConfigurator {
    ...
    private val profilePrefix = if (StringUtils.isBlank(profile)) "" else "/profile=\"${profile.run(StringUtilsImpl::escapeStringForCLICommand)}\""
                        ...
                        if (!it.isSuccess) {
                            ...
                            this.runCommandExpectSuccess(
                                    "$profilePrefix/subsystem=elytron/key-store=${KEYSTORE_NAME}:add(" +
                                            "path=\"${options.createKeystore().run(StringUtilsImpl::escapeStringForCLICommand)}\", " +
                            ...
                            this.runCommandExpectSuccess(
                                    "$profilePrefix/subsystem=elytron/key-store=${KEYSTORE_NAME}:write-attribute(name=path, " +
                                            "value=\"${options.createKeystore().run(StringUtilsImpl::escapeStringForCLICommand)}\")",
