...
class TomcatDeploy {
    ...
    fun deployArtifact(options: TomcatOptions) {
        ...
        RetryServiceImpl.createRetry().execute(RetryCallback<Unit, Throwable> { context ->
            println("Attempt ${context.retryCount + 1} to deploy ${options.application} to ${options.deployUrl.toExternalForm()}")
            ...
            Try.Success(generateExecttor(options))
                    ...
                    .onSuccess { println("Application deployed successfully") }
    ...
    fun redeployArtifact(options: TomcatOptions) {
        ...
        RetryServiceImpl.createRetry().execute(RetryCallback<Unit, Throwable> { context ->
            println("Attempt ${context.retryCount + 1} to deploy ${options.tag} to ${options.redeployUrl.toExternalForm()}")
    ...
    fun undeployArtifact(options: TomcatOptions) {
        ...
        RetryServiceImpl.createRetry().execute(RetryCallback<Unit, Throwable> { context ->
            println("Attempt ${context.retryCount + 1} to ${if (options.deploy) "deploy ${options.tag}" else "undeploy ${options.application}"} to ${options.undeployUrl.toExternalForm()}")
            ...
            Try.Success(generateExecttor(options))
                     ...
                     */
                    .map { response -> validateResponse(response) }
                    .onSuccess { println("Application undeployed successfully") }
    ...
    fun setDeploymentState(options: TomcatOptions) {
        ...
        RetryServiceImpl.createRetry().execute(RetryCallback<Unit, Throwable> { context ->
             println("Attempt ${context.retryCount + 1} to ${if (options.enabled) "start" else "stop"} ${options.application} via ${url.toExternalForm()}")
            ...
            Try.Success(generateExecttor(options))
                     ...
                     */
                    .map { response -> validateResponse(response) }
                    .onSuccess { println("Application ${if (options.enabled) "started" else "stopped"} successfully") }
